...
class BayesLinear_Normalq(nn.Module):
    ...
    def forward(self, X, sample=False):
        # print(self.training,sample)
        self.training = int(os.environ["MODE"])
...
class BBP_Bayes_Net(BaseNet):
    ...
    def create_net(self):
        ...
        print('    Total params: ',(self.get_nb_parameters()))
    ...
    def eval(self, x, y, train=False):
        ...
        pred = out.data.max(dim=1, keepdim=False)[1]  # get the index of the max log-probability
        err = pred.ne(y.data).sum()
        F_score = f1_score(y.data.cpu(),pred.cpu(),average='macro'); print("F1 score:",F_score)
